      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <h2 className="text-sm font-black uppercase tracking-[0.4em] text-white/20 mb-8 flex items-center gap-3">
            ...
            {trends.map((trend, i) => (
              <motion.div 
                ...
                className="glass-panel p-8 rounded-[2.5rem] border border-white/10 hover:border-[var(--color-neon-cyan)]/30 transition-all group relative overflow-hidden"
              >
                <div className="flex flex-col md:flex-row justify-between gap-8 items-center relative z-10">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-1 rounded border ${
                        trend.status === "Hype" ? "bg-orange-500/10 border-orange-500/30 text-orange-400" :
                  ...
                  <div className="flex items-center gap-12 shrink-0">
                    <div className="text-center">
                      <div className="text-3xl font-black text-white">{trend.demand}%</div>
                      <div className="text-[8px] font-black uppercase tracking-widest text-white/20 mt-1">Market Demand</div>
                    </div>
                    <div className="text-center">
                      <div className={`text-xl font-black flex items-center justify-center gap-1 ${trend.growth > 0 ? "text-green-400" : "text-red-400"}`}>
                        {trend.growth > 0 ? <ArrowUpRight className="w-4 h-4" /> : <ArrowDownRight className="w-4 h-4" />}
                        {Math.abs(trend.growth)}%
                      </div>
                      <div className="text-[8px] font-black uppercase tracking-widest text-white/20 mt-1">YoY Growth</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 h-1 bg-white/5">
                  <motion.div 
                    ...
                    className={`h-full ${trend.growth > 0 ? "bg-green-400/30" : "bg-red-400/30"}`}
                  />
                </div>
              </motion.div>
            ))}
